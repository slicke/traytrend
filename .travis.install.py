...
def install_lazarus_version(ver,rel,env):
    ...
    if osn == 'wine':
        ...
        if os.system('sudo sh -c "echo \'foreign-architecture i386\' > /etc/dpkg/dpkg.cfg.d/multiarch" && %s install ia32-libs && %s update && %s install xvfb wine' % (OS_PMAN, OS_PMAN, OS_PMAN)) != 0:
            return False
